...
function getpage($count, $pagesize = 10) {
    ...
    $p->setConfig('header', '<span>共<b>%TOTAL_ROW%</b>条记录/共<b>%TOTAL_PAGE%</b>页</span>');
...
function getAjaxPage($count, $pagesize = 10) {
    $p = new Think\PageAjax($count, $pagesize);
    $p->setConfig('header', '<span>共<b>%TOTAL_ROW%</b>条记录/共<b>%TOTAL_PAGE%</b>页</span>');
    //$p->setConfig('header', '<span>共<b>%TOTAL_ROW%</b>条记录&nbsp;第<b>%NOW_PAGE%</b>页/共<b>%TOTAL_PAGE%</b>页</span>');
    $p->setConfig('prev', '上一页');
    $p->setConfig('next', '下一页');
 ...
 * @return array        excel文件内容数组
 */
function import_excel_new($file, $type){
    // 判断文件是什么格式
    if($type == 'csv')
        $type = 'CSV';
    else if($type == 'xls')
...
function geetest_chcek_verify($data){
    ...
    function writeLog($str, $fileName='log.txt'){
        $root=$_SERVER['DOCUMENT_ROOT'].'/Uploads';
        if(!is_dir($root)){
            mkdir($root);
        }
        $rootLog = $_SERVER['DOCUMENT_ROOT'].'/Uploads/log';
        if(!is_dir($rootLog)){
            mkdir($rootLog);
        }
    ...
    */
    function format_money($money){
        if(floatval($money)==0)
            return 0;
        $str = preg_replace("/(?=\d)(?!\.\d*)(\d)(?=(?:\d{3})+($|\.\d*))/","$1,",$money);
        return strpos($str,'.')===false?$str:preg_replace("/\.?0*$/","",$str);
    } 
	...
	/**
     * 快速生成订单号
     * @param $type 订单类型；redpacket:红包赔付
     * @param $uid 暂为用户手机号
     * @returns {*} 订单号
     */
	function fastGenerate($type='W') {
    	//订单首字母类型
    	$prexArr=array(
    		'financeWithdraw'=>$type
		);
    	$prex=isset($prexArr['financeWithdraw'])?$prexArr['financeWithdraw']:"";

        $randomUID = randNumber(2);
        $randomOID = randNumber(1);
        $no = makeOrderNo($randomUID, $randomOID, array('uidLength'=>strlen($randomUID)));
        $round = explode(' ',microtime());
        return $prex.date('ymdHis').round($round[0]*1000).$no;
    }
    
    function makeOrderNo($uid, $oid,$len) {
        $uidLength=isset($len['uidLength'])?$len['uidLength']:2;
        $oidLength=isset($len['oidLength'])?$len['oidLength']:3;
        $randomLength=isset($len['randomLength'])?$len['randomLength']:2;
       
        return fillOrSubToFixed($uid, $uidLength)
        .fillOrSubToFixed($oid, $oidLength)
        .randNumber($randomLength);
    }
    /**
    ...
    function fillOrSubToFixed($sNum, $sLength) {
        return substr($sNum,$sLength).randNumber($sLength-strlen($sNum));
    }
    function randNumber ($len) {
        return sprintf("%0".$len."d", mt_rand(10,99));
    }
    /**
     * //以数组某个键为索引
     ...
     */
    function array_key_switch($src_array, $switch_key) {
		$dest_array = array();
		foreach ($src_array as $value) {
			$dest_array[$value[$switch_key]] = $value;
		}
		return $dest_array;
	}
    /**
    * 验证数字和价格
    * @param array $data array('content'=>1,'type'=>num,'msg'=>'数量填写错误')
    * @return array('code'=>200);|| array('code'=>201,'msg'=>$msg);
    */
    function verift_money_num($data){
        if(!is_array($data))
            return array('code'=>201,'msg'=>'验证信息格式不正确');
        $return = array('code'=>200);
        foreach($data as $key => $val){
            if($val['type'] == 'num'){
                if(!preg_match_all('/^\d+$/', $val['content'], $matches)){
                    $return = array('code'=>201,'msg'=>$val['msg']);
                    break;
                }
            }else if($val['type'] == 'money'){
                if(!preg_match_all('/^\d+.?\d{0,2}$/', $val['content'], $matches)){
                    $return = array('code'=>201,'msg'=>$val['msg']);
                    break;
                }
            }
    ...
    /**
     * 构建excel列
     * @param $startChar：起始列：A-YZ
     * @param $length：要从起始列构建的列数
     * @return ['A','B','C',...'YZ']
     */
    function buildExcelColumns($startChar,$length){
        if($startChar>'Z'||$startChar<'A')
            //起始列必须包含在A-YZ
            return false;
        $e=$startChar?$startChar:'A';
        $length=$length>0?$length:0;
        $return =[];
        $inx=0;
        for($e=$startChar;$e<='Z';$e++){
            // echo ord($e).'：'.$e."\r\n";
            $return[]=$e;
            if(++$inx>=$length)
                break;
    ...
    /**
    * 格式化重量单位：$val为整型数字,$val单位限定为g
    * 返回结果:>1kg则显示*KG；<1kg，则显示*g
    */
    function formatWeight($val){
        ...
        if($val<1000){
            ...
            return round(floatval($val)/1000,2).'kg';
        }
    }
    /**
    * 数据加密
     */
    function dataEncryptionBy128CBC($privateKey, $iv = '', $data) {
        $data = addPKCS7Padding($data);
        $encrypt_str =  mcrypt_encrypt(MCRYPT_RIJNDAEL_128, $privateKey, $data, MCRYPT_MODE_CBC, $iv);
        return $encrypt_str;
    }
    /**
     * 填充算法
    ...
    */
    function addPKCS7Padding($source){
        ...
        $pad = $block - (strlen($source) % $block); //补满要加密的数据到加密快长度的整数
        if ($pad <= $block) {
            $char = chr($pad);
     ...
     * 数据解密
     */
    function dataDecryptionBy128CBC($privateKey, $iv = '', $data) {
        //AES, 128 ECB模式加密数据
        $str = base64_decode($data);
        $encrypt_str =  mcrypt_decrypt(MCRYPT_RIJNDAEL_128, $privateKey, $str, MCRYPT_MODE_CBC, $iv);
     ...
     * 格式化时间戳，精确到毫秒，x代表毫秒 
     */
    function formatMillisecond($tag, $time) {
        list($usec, $sec) = explode('.', $time/1000);
        $date = date($tag,$usec);
        return str_replace('x', $sec, $date);
    }
    /**
    * 根据文件名返回文件类型，未知则返回空：image/office/pdf/text
    */
    ...
    * 人民币（分）转换为人民币（元）
    */
    function getYuan($val){
        $val = sprintf("%.2f",$val/100.0);
        return $val;
    }
    /**
    * 人民币（元）转换为人民币（分）
    ...
    */
    ...
    /**
     * [getDictValue 根据字典项类型、名称返回字典项值
     *   
     * @date   2017-09-12
     * @param  [string]     $type  [字典项类型]
     * @param  [string]     $title [字典项名称]
     * @param  [string]     $field [字典项字段]
     * @return [string]            [description]
     */
    function getDictValue($type, $title, $field = 'value'){
        $val = '';
        if (!$type && !$title) {
            return '';
        }
        ...
        $val = M('dict')->where($condition)->getField($field);
        return $val;
    }
    /**
    * 单个File组件上传多个文件时，将所有文件格式化成数组
    */
    ...
    function purchaseStatus($status,$payment_type=100){
